...
- `geezipx decompress archive.tar.zst -o /tmp/out` — 指定输出目录（支持 tar.gz、tar.zst 等归档格式）
- `geezipx decompress archive.zip --stdout` — 解压到标准输出（仅 gzip/zstd 单流格式支持；tar.gz、tar.zst 等多文件归档使用 `--stdout` 时报错）
