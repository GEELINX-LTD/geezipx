...
| 特性 | 说明 |
|------|------|
| 格式支持 | `.tar.gz`, `.tgz`, `.zip`（读/写）；`.tar`, `.gz`, `.tar.xz`, `.tar.zst`（读/写，其中 xz/zstd 依赖系统库） |
| 流式处理 | 文件流读写，内存占用与文件大小解耦 |
| 进度显示 | tqdm 风格进度条，支持 `--progress` / `--no-progress` |
| 格式自动检测 | 根据文件魔数（magic bytes）自动检测归档格式 |
| 压缩级别 | `--level 0-9`，控制 gzip 和 deflate 等级 |
| 标准管道 | 支持 stdin/stdout，兼容 Unix 管道链 |
| 递归操作 | `-r` 递归添加目录，保持目录结构 |
| 覆盖保护 | `--no-clobber` / `--clobber` 策略 |
| 测试覆盖 | 核心逻辑测试覆盖率 > 80%，集成测试覆盖主要格式 |
| 三平台 CI | GitHub Actions：Ubuntu, macOS, Windows 全自动构建与测试 |
