...
| 操作 | 当前格式 | 目标扩展 |
|------|----------|----------|
| 压缩 | ZIP、TAR、TAR.GZ、GZIP、ZSTD、TAR.ZST、XZ、TAR.XZ、LZMA | 7z 写入、LZH、ISO、ZIPX、SFX、ZPAQ（后续阶段） |
| 解压缩 | 上述所有格式 + 7z 只读 + RAR 只读 | CAB、ARJ、LHA、ACE、BZ2、BR、LZ4、DEB、WIM 等（后续阶段，含历史格式适配器） |

> 完整格式目标清单见 `docs/PRD.md` 第 5.1 节。新增格式按 feature gate 引入，不要求当前版本一次性完成。

GUI 中 7z/RAR 保持只读语义：可浏览、测试、提取，不可创建。
