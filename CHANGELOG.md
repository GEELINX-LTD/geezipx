...
- **GUI build**: Fixed Svelte 5 release build failure caused by invalid `$state()` rune usage in object literal properties. Converted `archiveManager` from object literal to class in `archiveStore.svelte.ts`, restoring the officially supported pattern for `$state` runes in Svelte 5.

### Changed

- **CI**: Added `gui-build` job to the PR CI pipeline (`ci.yml`) to catch Svelte/Vite frontend compilation errors before release.

## [0.7.1] - 2026-07-12

### Added

- **GUI**: Multi-archive tab browsing with tab-level archive management and cold-start file open support.
