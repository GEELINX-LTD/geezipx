//! Tar.zst (tarball compressed with Zstandard) reader and writer.
//!
//! Combines [`tar`] and [`zstd`] to produce/consume a zstd-compressed
...
impl<R: Read + Seek + Send> TarZstReader<R> {
    ...
    /// in a new zstd decoder on each pass.
...
/// Collect tar entries from a zstd-decoded stream.
...
/// Zstandard-compressed TAR archive writer.
///
/// Generic over any `W: Write + Send`.  Data is first tar-ed, then
/// zstd-compressed, and finally written to the underlying writer.
/// Construct via [`TarZstWriter::new`], add entries with
/// [`add_entry_from_reader`](ArchiveWriter::add_entry_from_reader),
/// then finalise with either:
///
/// - [`TarZstWriter::finalize`] — returns `(total_bytes, inner_writer)`
/// - [`ArchiveWriter::finish`] — returns `total_bytes` (trait object-safe)
pub struct TarZstWriter<W: Write + Send> {
    // Chained: tar::Builder writes into zstd Encoder, which writes into
...
impl<W: Write + Send> TarZstWriter<W> {
    /// Create a new tar.zst writer targeting the given output with the
    /// specified zstd compression level.
    ///
    /// `level` controls the zstd compression strength (0..=22). `None` uses the
    /// zstd default level (0 = default in zstd API, currently 3).
    pub fn new_with_level(writer: W, level: Option<u32>) -> Self {
        let level_i32 = match level {
            None | Some(0) => 0, // zstd uses 0 as "default"
            Some(l) => l as i32,
        };
    ...
    /// Create a new tar.zst writer targeting the given output using the
...
mod tests {
    ...
    /// Create a minimal valid tar.zst archive in memory.
    ...
    /// Create a raw tar.zst archive where the tar entry has `path`
    /// (allowing malicious paths that `tar::Header::set_path` rejects).
    /// We construct the tar bytes manually, then zstd-compress them.
    ...
    fn tarzst_unicode_filename() {
        ...
        let data = create_test_tarzst(&[("\u{4e2d}\u{6587}.txt", content)]); // 中文.txt
    ...
    fn tarzst_empty_archive() {
        // A minimal tar.zst with no entries: use the writer.
    ...
    fn tarzst_extract_all_with_dir_entries() {
        // Build a tar.zst with an explicit directory entry plus a file inside
...
fn tarzst_truncated_not_panic() {
    // Only valid zstd magic bytes (28 b5 2f fd), no tar data at all.
