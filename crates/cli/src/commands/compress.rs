...
fn validate_compress_inputs(
    ...
    if format == ArchiveFormat::Deb {
        anyhow::bail!(
            "deb writing is not supported; use list, test, or decompress for read-only deb support"
        );
    }
    ...
    if format == ArchiveFormat::Zpaq {
        anyhow::bail!(
            "zpaq writing is not supported; use list, test, or decompress for read-only zpaq support"
        );
    }
