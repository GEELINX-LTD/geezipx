...
/// Infer the decompressed filename for a BIN file by stripping `.bin` from
/// the filename.
pub fn bin_output_filename(archive: &Path) -> PathBuf {
    let name = archive
        .file_name()
        .map(|s| s.to_string_lossy().to_string())
        .unwrap_or_else(|| "output".to_string());
    let stripped = name.strip_suffix(".bin").unwrap_or(&name);
